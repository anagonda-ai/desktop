...
class PairwiseScorerProcessor(BioinformaticsProcessor):
    """Migrated from legacy script: pairwise_scorer.py."""
...
def process_pathways(json_file, fasta_file, output_dir):
    ...
    if not pathways:
        logger.info("No valid pathways with 'window_cluster_genes' found.")
        return
    ...
    for pathway in pathways:
        ...
        logger.info(f"Processing pathway: {pathway['pathway']} ({len(window_genes)} genes)...")
        ...
        logger.info(f"Results saved in '{pathway_dir}'.")

    logger.info("Processing complete. All results are stored in:", output_dir)
