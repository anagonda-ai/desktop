...
def process_genome_csv(filepath):
    ...
    try:
        ...
        logger.info(f"❌ Failed on {filepath}: {e}")
    return records

def build_merged_fasta():
    if os.path.exists(MERGED_FASTA):
        logger.info(f"✅ Merged FASTA already exists: {MERGED_FASTA}")
        return

    logger.info("📦 Building merged FASTA with concurrency...")
    genome_csvs = []
    ...
    logger.info(f"✅ Wrote merged FASTA: {MERGED_FASTA}")
...
def build_blast_db():
    ...
    if db_files_exist:
        logger.info(f"✅ BLAST DB already exists: {BLAST_DB}")
        return

    logger.info("🛠️  Building BLAST DB...")
    subprocess.run([
        "makeblastdb",
        "-in", MERGED_FASTA,
        "-dbtype", "prot",
    ...
    logger.info("✅ BLAST DB created.")


# === STEP 3: Run BLAST ===
def run_blast():
    if os.path.exists(BLAST_OUT):
        logger.info(f"✅ BLAST results already exist: {BLAST_OUT}")
        return
...
def save_distances_to_csv(distances, output_file):
    ...
    logger.info(f"✅ MGC index distances saved to: {output_file}")

def load_final_output_and_save_distances_to_csv():
    if not os.path.exists(FINAL_OUTPUT):
        logger.info(f"❌ Final output file does not exist: {FINAL_OUTPUT}")
        return
