...
class FindClusterInChromosomesProcessor(BioinformaticsProcessor):
    """Migrated from legacy script: find_cluster_in_chromosomes.py."""
...
def select_highest_score_genes(df):
    ...
    if 'bit_score' not in df.columns:
        logger.info("Warning: Required column 'bit_score' not found. Using all genes as-is.")
        return df
...
def process_organism(organism, file_paths, output_dir, all_distances):
    ...
    for file_path in file_paths:
        ...
        try:
            ...
            logger.info(f"Skipping invalid or malformed file: {file_path}")
            continue
        ...
        if 'chromosome' not in df.columns:
            logger.info(f"Skipping file {file_path}, missing required columns.")
            continue
    ...
    for chrom, rows in chromosome_data.items():
        if len(rows) < 2:
            logger.info(f"Skipping chromosome {chrom} for {organism}, not enough data.")
            continue
    ...
    if not all_genes_df.empty:
        ...
        logger.info(f"Saved cross-chromosome clusters: {cross_chrom_output_file}")
...
def process_csv_files(root_dir):
    ...
    if all_distances:
        ...
        logger.info(f"Saved general summary statistics: {stats_output_file}")
...
def _parse_blast_file(file_path):
    ...
    try:
        ...
        return hits
    logger.info(f"Parsing {file_path} with {len(df)} rows")
    for _, row in df.iterrows():
        try:
            sseqid = str(row[1])
...
def _tightest_cluster_for_chromosome(args):
    ...
    logger.info(f"[Organism {organism} | Chromosome {chrom}] Processing {len(hits)} hits...")
    ...
    n = len(origins)
    if n < 2:
        logger.info(f"[Organism {organism} | Chromosome {chrom}] Skipped: less than 2 origins.")
        return None
    best_cluster = None
    best_span = None
    best_size = 0
    for group_size in range(n, 1, -1):
        logger.info(f"[Organism {organism} | Chromosome {chrom}] Trying group size {group_size}...")
        found = False
        for origin_subset in itertools.combinations(origins, group_size):
            ...
            for combo in itertools.product(*groups):
                ...
                if (best_span is None) or (group_size > best_size) or (group_size == best_size and span < best_span):
                    ...
                    found = True
        if found:
            logger.info(f"[Organism {organism} | Chromosome {chrom}] Found cluster with {best_size} genes, span {best_span}.")
            break
    if best_cluster is not None and len(best_cluster) >= 2:
        ...
        logger.info(f"[Organism {organism} | Chromosome {chrom}] Saved tightest cluster: {tightest_file}")
    ...
    logger.info(f"[Organism {organism} | Chromosome {chrom}] No valid cluster found.")
    return None
...
def compute_tightest_clusters_from_raw_blast(directory, output_dir, max_workers=30):
    ...
    logger.info(f"[Main] Parsing {len(files)} BLAST result files with up to {max_workers} workers...")
    ...
    logger.info(f"[Main] Parsed hits for {len(all_hits_chromosome_data)} (organism, chromosome) pairs. Starting tightest cluster search...")
    ...
    logger.info(f"[Main] Finished tightest cluster search for all (organism, chromosome) pairs.")
    ...
    for org, info in best_by_organism.items():
        ...
        logger.info(f"[Main] Saved best tightest cluster for organism {org}: {best_file}")
