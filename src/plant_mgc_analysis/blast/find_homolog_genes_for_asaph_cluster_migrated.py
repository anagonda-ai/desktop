...
class FindHomologGenesForAsaphClusterProcessor(BioinformaticsProcessor):
    """Migrated from legacy script: find_homolog_genes_for_asaph_cluster.py."""
...
def csv_to_fasta(csv_path):
    ...
    if os.path.exists(fasta_path):
        logger.info(f"⚠️ Skipped existing FASTA: {fasta_path}")
    else:
        ...
        if valid_entries == 0:
            ...
            logger.info(f"⚠️ Skipped empty CSV: {csv_path}")
            return None

        logger.info(f"✅ Converted {csv_path} → {fasta_path}")
    return fasta_path
...
def create_blast_db(fasta_file):
    ...
    try:
        if os.path.exists(db_name + ".pin"):
            logger.info(f"⚠️ Skipped existing BLAST DB: {db_name}")
        else:
...
def run_blastp(query_fasta, csv_file, blast_db):
    ...
    try:
        if os.path.exists(output_file):
            logger.info(f"⚠️ Skipped existing BLASTP results: {output_file}")
        else:
            subprocess.run(cmd, shell=True, check=True)
...
def save_best_hits(best_hits_by_organism, output_dir):
    ...
    for organism, hits in best_hits_by_organism.items():
        ...
        logger.info(f"✅ Best hits saved for organism: {organism}")
...
logger.info("🚀 Processing completed successfully with intelligent analysis!")
