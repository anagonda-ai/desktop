...
class SlidingWindowProcessor(BioinformaticsProcessor):
    """Migrated from legacy script: sliding_window.py."""
...
def read_fasta(fasta_file):
    ...
    logger.info(f"Loaded {len(genes)} genes from FASTA file.")
    return pd.DataFrame(genes)

def read_blast_scores(blast_file):
    """Reads a BLAST score file and returns a DataFrame."""
    logger.info(f"Reading BLAST scores file: {blast_file}")
    blast_df = pd.read_csv(blast_file, sep='\t')
    logger.info(f"Loaded {len(blast_df)} rows of BLAST scores.")
    return blast_df
...
def create_sliding_windows(genes_df, window_size, blast_df):
    ...
    logger.info(f"Creating sliding windows with size {window_size}.")
    windows = []
    current_window = []
    seen_genes = set()
...
def filter_windows_parallel(windows, blast_df, score_threshold, min_genes_above_threshold, num_workers):
    ...
    logger.info(f"Filtering {len(windows)} windows in parallel with {num_workers} workers.")
    filtered = []
    ...
    logger.info(f"Filtered down to {len(filtered)} windows.")
    return filtered
...
def blast_window_against_mgc(window, mgc_fasta_path, window_dir, window_idx, mgc_id):
    ...
    logger.info(f"BLAST completed for window {window_idx} against MGC {mgc_id}.")
    return blast_output
...
def split_mgc_fasta(mgc_fasta, output_dir):
    ...
    logger.info(f"Splitting MGC FASTA file: {mgc_fasta}")
    ...
    for mgc_id, sequences in mgc_groups.items():
        ...
        logger.info(f"Saved {len(sequences)} sequences for MGC {mgc_id}.")
...
def blast_windows_against_mgcs(filtered_windows, mgc_dir, output_dir):
    ...
    logger.info(f"Blasting {len(filtered_windows)} windows against MGC-specific FASTA files in {mgc_dir}.")
    os.makedirs(output_dir, exist_ok=True)
