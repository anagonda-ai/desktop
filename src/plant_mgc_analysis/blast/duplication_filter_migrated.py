...
class DuplicationFilterProcessor(BioinformaticsProcessor):
    """Migrated from legacy script: duplication_filter.py."""
        ...
        for fasta_filename in os.listdir(candidate_directory):
            ...
            for csv_filename in os.listdir(mgc_directory):
                ...
                if len(matching) == len(candidate_sequences):
                    logger.info(f"✅ All sequences in {fasta_path} matched to {csv_path}.")
            ...
            if not all_match_found:
                logger.info(f"❌ No matches found for {fasta_path} in any MGC CSV file.")
...
def group_and_filter_redundant_files(results, fasta_map, blast_output_dir):
    ...
    logger.info(f"📄 Deduplicated file list saved to: {dedup_file}")
...
def blast_all_vs_all_slurm(merged_list, output_root):
    ...
    for q_path, q_fasta in fasta_map.items():
        ...
        while True:
            ...
            if running_jobs < 100:
                break
            logger.info(f"⏳ Waiting: {running_jobs} jobs running for 'alongonda'. Sleeping 30s...")
            time.sleep(30)
        sbatch_cmd = [
            "sbatch",
            "--job-name", job_name,
        ...
        if result.returncode == 0:
            logger.info(f"✅ Submitted SLURM job for {q_name}: {result.stdout.strip()}")
        else:
            logger.info(f"❌ Failed to submit job for {q_name}: {result.stderr.strip()}")
