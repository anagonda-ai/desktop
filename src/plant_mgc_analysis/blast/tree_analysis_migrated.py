...
class TreeAnalysisProcessor(BioinformaticsProcessor):
    """Migrated from legacy script: tree_analysis.py."""
...
def analyze_tree_clades_dynamic(
    ...
    logger.info("Loading comparison table")
    ...
    logger.info(f"Matched {len(match_list)} entries from comparison table to tree")
    ...
    try:
        ...
        logger.info(f"✅ Global CladePP Score: {global_score}")
        ...
        logger.info(f"⚠️ Skipping global CladePP due to error: {e}")
        global_score = None
    ...
    for clade in tree.get_nonterminals():
        ...
        try:
            ...
            logger.info(f"Skipping clade {clade_id} due to error: {e}")
        clade_id += 1

    df = pd.DataFrame(result_rows)
    df.to_csv(f"{output_prefix}_summary.csv", index=False)
    logger.info(f"Saved clade correlation summary to {output_prefix}_summary.csv")
