...
class FilterMitochondrialGenesProcessor(BioinformaticsProcessor):
    """Migrated from legacy script: filter_mitochondrial_genes.py."""
...
def run_blastp_filter(input_csv, output_csv):
    ...
    if 'sequence' not in df.columns:
        logger.info(f"⚠️ Skipping {input_csv} - no 'sequence' column")
        return
    ...
    if not records:
        logger.info(f"⚠️ No protein sequences in {input_csv}")
...
logger.info("✅ All files filtered. Output saved to:", OUTPUT_DIR)
