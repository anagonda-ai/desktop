...
class KeggScannerMinGenesBasedProcessor(BioinformaticsProcessor):
    """Migrated from legacy script: kegg_scanner_min_genes_based.py."""
...
def blast_and_map_to_kegg(genome_file, kegg_db, temp_dir, identity_threshold=90.0, evalue_threshold=1e-3, coverage_threshold=90.0):
    ...
    logger.info(f"Running BLASTP for {genome_file} against KEGG database...")
    ...
    logger.info(f"BLASTP completed for {genome_file}.")

    # Parse BLAST results
    id_to_pathway = {}
    id_to_annotation = {}
    if os.path.exists(blast_output):
        logger.info(f"Parsing BLAST results for {blast_output}...")
...
def annotate_genomes_once(genome_dirs, kegg_db, temp_dir, annotated_dir, max_workers=32):
    ...
    for genome_dir in genome_dirs:
        logger.info(f"Searching for genome files in {genome_dir}...", flush=True)
        for file in os.listdir(genome_dir):
            if file.endswith('.csv'):
                genome_files.append(os.path.join(genome_dir, file))

    logger.info(f"Found {len(genome_files)} genome files to annotate.")

    def annotate_single_file(genome_file):
        logger.info(f"Processing file: {genome_file}", flush=True)
        filename = os.path.basename(genome_file).replace('.csv', '_annotated.csv')
        output_path = os.path.join(annotated_dir, filename)

        if os.path.exists(output_path):
            logger.info(f"Annotated file already exists: {output_path}, skipping.", flush=True)
            return

        try:
            logger.info(f"Starting BLAST for {genome_file}...", flush=True)
            df_annotated = blast_and_map_to_kegg(genome_file, kegg_db, temp_dir)
            df_annotated.to_csv(output_path, index=False)
            logger.info(f"Finished BLAST and saved {output_path}", flush=True)
        except subprocess.CalledProcessError as e:
            logger.info(f"BLAST failed for {genome_file}: {e}", flush=True)
        except Exception as e:
            logger.info(f"Unexpected error for {genome_file}: {e}", flush=True)
...
def process_annotated_file(annotated_file, output_file, file_lock, window_size, min_genes):
    logger.info(f"Processing file: {annotated_file} with window size: {window_size}")
    total_matches = 0
            ...
            if len(window) >= min_genes:
                ...
                if pathway and not tuple(metabolic_genes) in prev_matches:
                    ...
                    with file_lock:
                        ...
                        total_matches += 1
            i += 1
    logger.info(f"Completed file: {annotated_file}, Matches Found: {total_matches}")
...
def merge_annotated_mibig(annotated_dir, annotated_files):    
    ...
    for annotated_file in annotated_files:
        try:
            ...
            logger.info(f"❌ Failed to process {annotated_file}: {e}")

    # Merge and export
    if filtered_dfs:
        merged_df = pd.concat(filtered_dfs, ignore_index=True)
        merged_df.to_csv(output_path, index=False)
        logger.info(f"✅ Merged filtered data saved to: {output_path}")
    else:
        logger.info("⚠️ No matching rows found across files.")
...
def remove_subset_results(output_file):
    if not os.path.exists(output_file):
        logger.info(f"File not found: {output_file}")
        return
    ...
    logger.info(f"Removed {len(to_remove)} subset results from {output_file} and saved to {filtered_output_file}")
...
def main():
    ...
    head_output_dir = f"/groups/itay_mayrose/alongonda/Plant_MGC/kegg_metabolic_output_g{min_genes}"
