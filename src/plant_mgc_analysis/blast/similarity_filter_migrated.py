...
class SimilarityFilterProcessor(BioinformaticsProcessor):
    """Migrated from legacy script: similarity_filter.py."""
    ...
    for fb in filter_by:
        if fb.is_dir():
            ...
            with open(temp, 'w') as out_file:
                ...
                logger.info(f'Combined {fb.stem} filter-by files into {temp.name}')
            temp_fb.append(temp)
        elif fb.is_file():
            logger.info(f'Found {fb.name} as a filter-by file')
            temp_fb.append(fb)
    return temp_fb
...
def create_blast_db(pt_genes_file, db_name):
    ...
    if result.returncode != 0:
        logger.info("makeblastdb error:", result.stderr)
...
def parse_blast_results(blast_output, id_map, evalue_threshold=0.001):
    ...
    try:
            ...
            try:
                ...
                if query_id in id_map:
                    ...
                    try:
                        ...
                        logger.info(f'Could not parse query ID: {query_id}')
                        continue
                ...
                logger.info(f'Error processing hit in query {blast_qresult.id}: {str(e)}')
                continue
    except Exception as e:
        logger.info(f'Error parsing BLAST results from {blast_output}: {str(e)}')
        ...
        try:
            ...
            logger.info(f'Text parsing also failed: {str(text_error)}')
            raise
    return hits
...
def process_target(target, output, main_path, filter_by, silent):
    ...
    logger.info(f'Filtered {len(all_hits)} out of {len(id_map)} sequences:')
    if silent:
        for k, v in hits.items():
            logger.info(f'\tFrom {k}: {len(v)} hits')
    else:
        for k, v in hits.items():
            logger.info(f"\tFrom {k}:\n\t\t{'    '.join(v)}")
    ...
    logger.info(f'Filtered sequences written to {output_file.name}')
...
def filter_by_similarity(targets_dir, filter_by, relative, output, silent=True):
    ...
    logger.info(f'Found {len(target_fastas)} target files in {targets_dir}')
