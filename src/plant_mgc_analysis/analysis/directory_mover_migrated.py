...
def main():
    ...
    try:
        ...
        # Display results
        if result.success:
            logger.info("✅ Directory moving completed successfully!")
            stats = mover.get_move_statistics()
            logger.info(f"   Directories found: {stats['directories_found']}")
            logger.info(f"   Directories moved: {stats['directories_moved']}")
            logger.info(f"   Directories failed: {stats['directories_failed']}")
            logger.info(f"   Total size moved: {stats['total_size_moved_mb']:.2f} MB")
            ...
            logger.info("❌ Directory moving failed!")
