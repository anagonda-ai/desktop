...
class CreateFoldseekDbForPdbFileProcessor(BioinformaticsProcessor):
    """Migrated from legacy script: create_foldseek_db_for_pdb_file.py."""
...
def merge_and_create_db(mgc_dir: Path):
    ...
    if not merged_pdb.exists():
        ...
        logger.info(f"[+] Merged {len(pdb_files)} into {merged_pdb.name}")
...
def main():
    ...
    logger.info(f"[*] Processing {len(mgc_dirs)} MGCs")
