...
def main():
    ...
    try:
        ...
        if result.success:
            ...
            logger.info(f"   Archives found: {stats['archives_found']}")
            logger.info(f"   Archives extracted: {stats['archives_extracted']}")
            logger.info(f"   Total files extracted: {stats['total_files_extracted']}")
            logger.info(f"   Archives deleted: {stats['archives_deleted']}")
            logger.info(f"   Success rate: {stats['success_rate']:.2%}")
            logger.info(f"   Processing time: {result.processing_time:.2f} seconds")
        else:
            logger.info("❌ Archive extraction failed!")
