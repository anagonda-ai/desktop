...
class GffSplicingHandlerProcessor(BioinformaticsProcessor):
    """Migrated from legacy script: gff_splicing_handler.py."""
...
def process_gff3_file(file_path):
    ...
    with open(out_gff, 'w') as fo, open(gene_mrna_out,'w') as fo2, open(mrna_canon_out,'w') as fo3:
      for feature in gff.all_features():
        ...
        logger.info("%s\t%s" %(gene['ID'][0], longest_transcript['ID'][0]),file=fo2)
        # print transcript - canon transcript matching
        for mrna in mrnas:
          logger.info("%s\t%s" %(mrna['ID'][0],longest_transcript['ID'][0]), file=fo3)
        ...
        gene.end = mrna_end
        logger.info(str(gene), file=fo)
        longest_transcript.start = mrna_start
        longest_transcript.end = mrna_end
        logger.info(str(longest_transcript), file=fo)
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(future_to_file):
            ...
            try:
                ...
                logger.info(f"Processing failed for {file_path} with exception: {exc}")
...
# Main function
def main():
    root_dir = "/groups/itay_mayrose/alongonda/datasets/full_genomes/phytozome/Phytozome/"
