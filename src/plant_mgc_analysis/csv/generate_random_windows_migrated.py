    ...
    def sample_windows(self) -> List[RandomWindow]:
        try:
            ...
            if len(chromosomes) == 0:
                logger.info(f"⚠️ No valid chromosomes in: {self.filepath}")
                return []
            ...
            for chrom in chromosomes:
                ...
                if len(chrom_df) < self.window_size:
                    logger.info(f"↪️ Skipping chromosome {chrom} in {self.filepath} (only {len(chrom_df)} genes)")
                    continue
            ...
            logger.info(f"❌ Error processing {self.filepath}: {e}")
            return []
...
class RandomWindowGenerator:
    ...
    def generate_all(self) -> pd.DataFrame:
        logger.info(f"🧬 Found {len(self.genome_files)} genome files.")
        windows = []
...
def main():
    ...
    if not df.empty:
        ...
        logger.info(f"✅ Saved {len(df)} random windows → {OUTPUT_PATH}")
    else:
        logger.info("⚠️ No random windows were generated.")
