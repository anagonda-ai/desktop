...
def main():
    ...
    try:
        ...
        # Display results
        if result.success:
            logger.info("✅ Column extraction completed successfully!")
            stats = extractor.get_processing_statistics()
            logger.info(f"   Files processed: {stats['files_processed']}")
            logger.info(f"   Rows extracted: {stats['rows_extracted']}")
            logger.info(f"   Rows failed: {stats['rows_failed']}")
            logger.info(f"   Success rate: {stats['extraction_success_rate']:.2%}")
            logger.info(f"   Processing time: {result.processing_time:.2f} seconds")
        else:
            logger.info("❌ Column extraction failed!")
