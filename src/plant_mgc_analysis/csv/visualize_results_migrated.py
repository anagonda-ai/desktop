...
class VisualizeResultsProcessor(BioinformaticsProcessor):
    """Migrated from legacy script: visualize_results.py."""
...
if 'source_file' in df.columns:
    ...
    for source_file, count in counts.items():
        ...
        try:
            ...
            logger.info(f"Could not read {source_path}: {e}")
            file_rows = None
            normalized = None

        cleaned_name = clean_filename(source_file)
        normalized_counts[cleaned_name] = normalized
        logger.info(f"Normalized count for {cleaned_name}: {normalized}")
    ...
    plt.show()
    logger.info(f"✅ Bar figure saved to: {bar_output_path}")
    ...
    plt.show()
    logger.info(f"✅ Scatter figure saved to: {scatter_output_path}")

else:
    logger.info("❌ No 'source_file' column found in the CSV.")
