...
class ComparisonCsvPipelineProcessor(BioinformaticsProcessor):
    """Migrated from legacy script: comparison_csv_pipeline.py."""
...
def wait_for_slot(max_jobs=100):
    while True:
        ...
        if running_jobs < max_jobs:
            break
        logger.info(f"⏳ Waiting: {running_jobs} jobs running. Sleeping 30s...")
        time.sleep(30)
...
def submit_job(job_name, out_file, err_file, script_path, mgc_path):
    ...
    if result.returncode != 0:
        logger.info(f"❌ Failed to submit job {job_name}: {result.stderr}")
        return None
    job_id = result.stdout.strip().split()[-1]
    logger.info(f"✅ Submitted {job_name} with job ID {job_id}")
    return job_id
...
def wait_for_job_and_file(job_id, expected_output_file):
    ...
    logger.info(f"⏳ Waiting for output file: {expected_output_file}")
    ...
    while True:
        ...
        prev_size = curr_size
        time.sleep(5)
    logger.info(f"✅ Output file ready: {expected_output_file}")
...
def process_mgc_path(mgc_path):
    ...
    try:
        ...
        logger.info(f"🎉 Finished all jobs for {mgc_name}")
    except Exception as e:
        logger.info(f"❌ Error processing {mgc_name}: {e}")
