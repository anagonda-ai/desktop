...
class VisualizeClustersProcessor(BioinformaticsProcessor):
    """Migrated from legacy script: visualize_clusters.py."""
...
if os.path.exists(MAPPING_FILE):
    ...
    mapping_df = None
    mapping_dict = {}
    logger.info("Warning: Dataset organism mapping file not found.")
...
if os.path.exists(COMPARISON_FILE):
    ...
    comparison_df = None
    logger.info("Warning: Comparison results file not found.")
            ...
            if text.get_text()[1:] == clade.name:
                try:
                    ...
                    text.set_color(color)
                        ...
                        logger.info(f"Invalid color: {clade.color}")
...
def plot_combined_tree_with_metrics(tree_file, output_path, metrics, metric_titles, chromosome_cluster_gene_existance, chromosome_cluster_gene_strands, cmap_name="viridis"):
    ...
    plt.close()
    logger.info(f"✅ Combined tree saved: {output_path}")
...
logger.info("✅ Phylogenetic trees saved successfully with separate metrics and scaled colors.")