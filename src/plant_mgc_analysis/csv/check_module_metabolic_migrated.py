...
class CheckModuleMetabolicProcessor(BioinformaticsProcessor):
    """Migrated from legacy script: check_module_metabolic.py."""
...
logger.info("Fetching KEGG organism list...")
...
logger.info(f"Found {len(plants)} plant organisms.")
...
def is_metabolic_module(module_id):
    ...
    try:
        ...
        logger.info(f"⚠️ Failed to fetch module {module_id}: {e}")
...
def fetch_gene_info(gene_batch, org_code):
    try:
        ...
        logger.info(f"Error fetching gene info: {e}")
...
def process_organism(org_code, org_name):
    ...
    if os.path.exists(org_folder) and any(fname.endswith(".csv") for fname in os.listdir(org_folder)):
        logger.info(f"⏩ Skipping {org_code} (already done)")
        return

    logger.info(f"Processing {org_code} - {org_name}")
    ...
    logger.info(f"✅ Done: {org_code}")

# Run for all plants
for code, name in plants.items():
    process_organism(code, name)

logger.info("✅ All metabolic module-based KEGG data collected.")
