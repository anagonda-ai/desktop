...
class PathwayGeneCounterProcessor(BioinformaticsProcessor):
    """Migrated from legacy script: pathway_gene_counter.py."""
...
def count_genes_in_unique_ids(csv_file):
    ...
    logger.info("Column names in the CSV file:", df.columns.tolist())
...
def main():
    ...
    for _, row in unique_id_gene_count.iterrows():
        logger.info(f"{row['Pathway']} - {row['Gene Count']} genes")
