...
class KeggMetabolicScannerSlurmIncludedProcessor(BioinformaticsProcessor):
    """Migrated from legacy script: kegg_metabolic_scanner_slurm_included.py."""
...
def main():
    ...
    for window_size in [10, 20]:
        ...
        logger.info("⏳ Waiting for all SLURM jobs to finish...")
        while True:
            running_jobs = count_running_jobs(user="alongonda", name_prefix="process_")
            if running_jobs == 0:
                logger.info("✅ All jobs completed.")
                break
            logger.info(f"⏳ {running_jobs} jobs still running. Waiting 60 seconds...")
            time.sleep(60)
        ...
        for tmp_file in tmp_files:
            try:
                ...
                logger.info(f"⚠️ Failed to read {tmp_file}: {e}")

        if merged_dfs:
            merged = pd.concat(merged_dfs, ignore_index=True)
            merged.to_csv(output_file, index=False)
            logger.info(f"✔️ Merged {len(tmp_files)} files into {output_file} (Total rows: {len(merged)})")
        else:
            logger.info("⚠️ No output files to merge.")
