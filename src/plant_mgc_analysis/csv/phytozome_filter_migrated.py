...
class PhytozomeFilterProcessor(BioinformaticsProcessor):
    """Migrated from legacy script: phytozome_filter.py."""
...
def process_unique_id(service, unique_id, gene_to_id):
    try:
        ...
        logger.info(f"Error processing unique_id {unique_id}: {e}")
    return None
...
def main():
    unique_ids_file_path = '/groups/itay_mayrose/alongonda/datasets/plantcyc/gene_by_db_seq/unique_dbs_for_unique_id.txt'
    unique_ids_seq = '/groups/itay_mayrose/alongonda/datasets/plantcyc/gene_by_db_seq/PHYTOZOME/unique_ids_seq.txt'
