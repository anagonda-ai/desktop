        ...
        for occurrence_idx, genes in occurrences.items():
            ...
            if len(genes) >= min_genes:
                ...
                if unique_tracker.is_unique_match(pathway_cluster):
                    ...
                    logger.info(f"Match Found: Pathway={pathway} (Occurrence {occurrence_idx}), "
...
def main():
    genome_dirs = [
        "/groups/itay_mayrose/alongonda/datasets/full_genomes/ensembl/processed_annotations_test_no_chloroplast_with_sequences/metabolic_genes",
        "/groups/itay_mayrose/alongonda/datasets/full_genomes/plaza/processed_annotations_with_chromosomes_no_chloroplast_with_sequences/metabolic_genes",
        "/groups/itay_mayrose/alongonda/datasets/full_genomes/phytozome/processed_annotations_with_chromosomes_no_chloroplast_with_sequences/metabolic_genes"
    ]
    ...
    for max_kbp in [20, 40, 60, 80, 100]:
        ...
        for genome_dir in genome_dirs:
            logger.info(f"Processing genome directory: {genome_dir} with window size: {max_kbp}kbp and min_genes: {min_genes}")
            total_matches += process_genome_dir(genome_dir, output_file, max_workers, min_genes, max_kbp)
        
        logger.info(f"TOTAL MATCHES FOUND for window size {max_kbp}kbp and min_genes {min_genes}: {total_matches}")
