...
def main():
    ...
    if len(sys.argv) == 1:
        ...
        logger.info("Usage: python kegg_pathway_extractor_local.py [source_folder] [output_folder]")
        logger.info("\nArguments:")
        logger.info("  source_folder  : Directory containing KEGG annotation CSV files")
        logger.info("  output_folder  : Directory for metabolic module output")
        logger.info("\nExample:")
        logger.info("  python kegg_pathway_extractor_local.py /data/kegg_modules /data/metabolic_modules")
    ...
    try:
        ...
        if result.status == ProcessingStatus.COMPLETED:
            ...
            if result.results:
                ...
                logger.info(f"✅ All metabolic module-based KEGG data collected.")
                logger.info(f"   Files processed: {stats['files_processed']}")
                logger.info(f"   Metabolic modules found: {stats['metabolic_modules_found']}")
