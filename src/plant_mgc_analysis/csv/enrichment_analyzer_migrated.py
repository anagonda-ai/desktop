...
class EnrichmentAnalyzerProcessor(BioinformaticsProcessor):
    """Migrated from legacy script: enrichment_analyzer.py."""
...
def main():
    ...
    if not csv_files:
        logger.info(f"No files matching pattern '{file_pattern}*.csv' found in {root_directory}")
        return
    ...
    logger.info(f"Ranking saved to {output_ranking_path}")
