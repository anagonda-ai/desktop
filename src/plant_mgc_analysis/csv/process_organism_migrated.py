...
def main():
    ...
    if len(sys.argv) < 4:
        logger.info("Usage: python process_organism.py <org_code> <root_folder> <metabolic_modules_path>")
        logger.info("\nArguments:")
        logger.info("  org_code               : KEGG organism code (e.g., 'hsa' for human)")
        logger.info("  root_folder           : Root directory for output files")
        logger.info("  metabolic_modules_path: Path to CSV file with metabolic module IDs")
        logger.info("\nExample:")
        logger.info("  python process_organism.py hsa /data/organisms modules.csv")
        sys.exit(1)
    ...
    try:
        ...
        if result.status == ProcessingStatus.COMPLETED:
            ...
            if result.results:
                ...
                logger.info(f"✅ {stats['organism_code']}: {stats['modules_processed']} modules, "
