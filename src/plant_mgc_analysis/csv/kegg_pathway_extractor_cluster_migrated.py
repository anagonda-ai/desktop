...
def main():
    ...
    if len(sys.argv) == 1:
        ...
        logger.info("Usage: python kegg_pathway_extractor_cluster.py [output_folder] [slurm_script_path]")
        logger.info("\nArguments:")
        logger.info("  output_folder      : Directory for output files and job results")
        logger.info("  slurm_script_path  : Path to SLURM script for organism processing")
        logger.info("\nExample:")
        logger.info("  python kegg_pathway_extractor_cluster.py /data/cluster ./scripts/kegg_download.sh")
    ...
    try:
        ...
        if result.status == ProcessingStatus.COMPLETED:
            ...
            if result.results:
                ...
                logger.info(f"✅ All metabolic module-based KEGG data collection initiated.")
                logger.info(f"   Plant organisms: {stats['plant_organisms_count']}")
                logger.info(f"   Metabolic modules: {stats['metabolic_modules_count']}")
                logger.info(f"   Jobs submitted: {stats['job_submission_stats']['successful']}")
                
                if stats['job_submission_stats']['failed'] > 0:
                    logger.info(f"   Failed submissions: {stats['job_submission_stats']['failed']}")
        else:
