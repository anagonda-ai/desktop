...
def process_file(file_path, pathway_tries, output_file, unique_tracker, file_lock, window_size, min_genes):
    ...
    with tqdm(total=num_windows, desc=f"Random Samples: {os.path.basename(file_path)}", unit="window") as pbar:
        for i in range(num_windows):
            window = df.sample(n=window_size)
            window_matches = process_window_with_aho_corasick(window, pathway_tries, unique_tracker, output_file, file_lock, file_path, min_genes)
            total_random_matches += window_matches
            pbar.update(1)
...
def main():
    ...
    output_dir = "/groups/itay_mayrose_nosnap/alongonda/Plant_MGC/sliding_window_outputs_chromosome_sorted_with_statistical_tests"
    ...
    for window_size in range(5, 21):
        ...
        for min_genes in [3]:
            ...
            enrichment_output_file = os.path.join(min_genes_subdir, f"enrichment_w{window_size}.csv")
